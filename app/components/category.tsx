    ...
    <section className="container md:py-15 py-10 md:px-10 px-3">
      <div className="border border-gray-400 rounded-3xl md:py-10 py-5 md:px-10 px-5">
        <div className="flex flex-col md:text-center">
          ...
          <p className="md:mt-6 mt-3 md:text-lg text-gray-700">“From design to delivery, every product reflects our commitment to quality.</p>
          <p className="md:text-lgtext-gray-700">durability, and customer satisfaction—because you deserve products that truly fit your lifestyle.”</p>
        </div>
        <div className="grid md:grid-cols-5 grid-cols-2 gap-6 mt-10 items-center w-full">
          {Data.CategoryItem.map((items) => (
            <div key={items.id} className="w-full relative md:h-76 h-30 hover:scale-105 ">
              <Image src={items.image} alt="category-image"
                fill
                unoptimized
                className="object-cover w-full md:h-60 h-40 relative shadow-xl"
                style={{ borderRadius: "70px 40px 80px 120px" }}
              />
